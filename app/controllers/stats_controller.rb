class StatsController < ApplicationController
  ...
  def index
    ...
    #stats.instance_variables.each do |iv|
    #  self.instance_variable_set iv, stats.instance_variable_get(iv)
    #end

    @me = stats.me
    @chart_width = stats.chart_width
    @chart_height = stats.chart_height
    @pie_width = stats.pie_width
    @pie_height = stats.pie_height
    @today = stats.today

    @cut_off_year = stats.cut_off_year
    @cut_off_year_plus3 = stats.cut_off_year_plus3
    @cut_off_month = stats.cut_off_month
    @cut_off_3months = stats.cut_off_3months

    @first_action = stats.first_action
    @tags_count = stats.tags_count
    @unique_tags_count = stats.unique_tags_count
    @hidden_contexts = stats.hidden_contexts

    @completed_actions = stats.completed_actions

    @actions_avg_ttc = stats.actions_avg_ttc
    @actions_max_ttc = stats.actions_max_ttc
    @actions_min_ttc = stats.actions_min_ttc

    @actions_min_ttc_sec = stats.actions_min_ttc_sec
    @actions_min_ttc_sec = stats.actions_min_ttc_sec

    @sum_actions_done_last30days = stats.sum_actions_done_last30days
    @sum_actions_created_last30days = stats.sum_actions_created_last30days

    @sum_actions_done_last12months = stats.sum_actions_done_last12months
    @sum_actions_created_last12months = stats.sum_actions_created_last12months

    @actions_per_context = stats.actions_per_context
    @running_actions_per_context = stats.running_actions_per_context
    @projects_and_actions = stats.projects_and_actions

    @projects_and_actions_last30days = stats.projects_and_actions_last30days

    @projects_and_runtime_sql = stats.projects_and_runtime_sql

    @projects_and_runtime = stats.projects_and_runtime
    @tags_for_cloud = stats.tags_for_cloud

    @tags_min = stats.tags_min

    @tags_divisor = stats.tags_divisor
    @tags_for_cloud_90days = stats.tags_for_cloud_90days

    @tags_min_90days = stats.tags_min_90days

    @tags_divisor_90days = stats.tags_divisor_90days
